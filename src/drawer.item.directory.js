...
class DrawerDirectory extends DrawerItem {
   ...
   getItem(type, compare) {
      let searchArray =
         type == "file" ? this.items.files : this.items.directories;
      let result = null;
   ...
   sort(type) {
      ...
      const sortedElements = elements.sort((a, b) =>
         b
            .querySelector(".drawer-text")
            .value.localeCompare(a.querySelector(".drawer-text").value)
      );
   ...
   addDirectory(title) {
      ...
      if (this.has("directory", title)) {
         if (this.root.options.warnings) {
            console.warn(
               `Directory '${title}' from path '${this.path}' already exists.`
            );
         }
   ...
   addFile(title) {
      ...
      if (this.has("file", title)) {
         if (this.root.options.warnings) {
            console.warn(
               `File '${title}' from path '${this.path}' already exists.`
            );
         }
   ...
   serialize(options = {}) {
      ...
      function deepscan(items, parentNode) {
         ...
         for (let directory of items.directories) {
            ...
            if (
               directory.items.directories.length != 0 ||
               directory.items.files.length != 0
            ) {
      ...
      function deepscan(data, parentDirectory) {
         ...
         if (data.directories) {
            ...
            for (let directory of data.directories) {
               ...
               // Continue scanning if either directories or files has an item in it
               if (
                  directory.directories != undefined ||
